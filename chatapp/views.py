...
def login(request):
	...
	if User.objects.filter(phone=user_name):
		db_user = User.objects.get(phone=user_name)
		# user found!
		if db_user.password != user_pass:
			# incorrect password
			notice = "oops " + db_user.phone + ", this password is incorrect!"
...
def create_user(request):
    ...
    if User.objects.filter(phone=user):
		# bad username
	    notice = "oops! phonenumber is already taken"
	    action = "please choose a differtent name"
	    
	    return render(request, "signup.html", {"notice": notice, "action": action})
    else:
		if password == password_conf:
			create_user = User(name=name, phone=user, password=password)
			create_user.save()
